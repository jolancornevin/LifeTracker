...
class LineChart extends React.Component<{
	...
	customValueRenderer: (index, point) => JSX.Element;
	primaryColor: string;
	...
	drawCoordinates(data, seriesColor, seriesIndex) {
		...
		for (let i = 0; i < dataLength - 1; i++) {
			result.push(
				this.drawCoordinate(i, data[i], data[i + 1], '#FFFFFF00', lineStyle, false, false, seriesIndex),
			);
		}

		if (dataLength > 0) {
			result.push(this.drawPoint(0, data[0], seriesColor));
			result.push(this.drawValue(0, data[0]));
		}

		for (let i = 0; i < dataLength - 1; i++) {
			result.push(this.drawPoint(i + 1, data[i + 1], seriesColor));
			result.push(this.drawValue(i + 1, data[i + 1]));
		}
					...
					<View style={StyleSheet.flatten([styles.selectedBox])}>
						{this.state.sortedData.map((series) => {
							...
							return dataObject.y ? (
								...
								<View key={series.seriesName}></View>
							);
						})}
					</View>
				</View>
